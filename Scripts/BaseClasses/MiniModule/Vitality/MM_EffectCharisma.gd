...
func _start_effect() -> void:
	...
	if apply_once and not apply_custom_effects_only:
		if change_base_value != 0:
				target_vitality.base_charisma += change_base_value
				if revert_on_end:
					_tracker_base_value += change_base_value
		
		if 0.0 < change_base_value_by_percentage:
			_calc_variable = change_max_value_by_percentage * float(target_vitality.base_charisma)
			target_vitality.base_charisma += floori(_calc_variable)
		...
		if change_max_value != 0:
			if apply_to == 0: # Apply to base
				target_vitality.max_charisma += change_max_value
			elif apply_to == 1: # Mod value
				target_vitality.mod_charisma += change_max_value

			if revert_on_end:
				_tracker_max_value += change_max_value
		
		if 0 < change_max_value_by_percentage:
				_calc_variable = float(target_vitality.max_charisma) * change_max_value_by_percentage
				
				if apply_to == 0:
					target_vitality.max_charisma += floori(_calc_variable)
				elif apply_to == 1:
					target_vitality.mod_charisma += floori(_calc_variable)
	...
	if not apply_custom_effects_only:
		if change_max_value_by_multiplier < 1.0:
			target_vitality.mult_charisma.append(change_max_value_by_multiplier)
			target_vitality.update_max_charisma()
...
func _apply_effect(Delta: float):
	...
	if not apply_once and not apply_custom_effects_only:
		...
		if 0 < change_base_value_by_percentage:
			_calc_variable = change_base_value_by_percentage * float(target_vitality.base_charisma) * Delta
			target_vitality.base_charisma += floori(_calc_variable)
		...
		if change_max_value_by_percentage != 0:
			_calc_variable = float(target_vitality.max_charisma) * change_max_value_by_percentage * Delta
...
func _end_effect():
	...
	if revert_on_end and not apply_custom_effects_only:
		if _tracker_base_value != 0:
			target_vitality.base_charisma -= _tracker_base_value
		
		if _tracker_max_value != 0:
			if apply_to == 0:
				target_vitality.max_charisma -= _tracker_max_value
			elif apply_to == 1:
				target_vitality.mod_charisma -= _tracker_max_value
		
		if _tracker_value_multiplier != 1.0:
			target_vitality.mult_charisma.erase(_tracker_value_multiplier)
			target_vitality.update_max_charisma()
