...
var base_health: float = 0 :
	set(value):
		if enabled:
			base_health = maxf(value, 1)
			max_health = ActorLibs.calculate_stati(base_health + skill_health, mod_health, mult_health)
var skill_health: float = 0:
	set(value):
		if enabled:
			skill_health = value
			max_health = ActorLibs.calculate_stati(base_health + skill_health, mod_health, mult_health)
var mod_health: float = 0 :
	set(value):
		if enabled:
			mod_health = value
			max_health = ActorLibs.calculate_stati(base_health + skill_health, mod_health, mult_health)
...
var base_stamina: float = 0:
	set(value):
		if enabled:
			base_stamina = maxf(value, 1)
			max_stamina = ActorLibs.calculate_stati(base_stamina + skill_stamina, mod_stamina, mult_stamina)
var skill_stamina: float = 0:
	set(value):
		if enabled:
			skill_stamina = value
			max_stamina = ActorLibs.calculate_stati(base_stamina + skill_stamina, mod_stamina, mult_stamina)
var mod_stamina: float = 0 :
	set(value):
		if enabled:
			mod_stamina = value
			max_stamina = ActorLibs.calculate_stati(base_stamina + skill_stamina, mod_stamina, mult_stamina)
...
var base_mana: float = 0:
	set(value):
		if enabled:
			base_mana = maxf(value, 1)
			max_mana = ActorLibs.calculate_stati(base_mana + skill_mana, max_mana, mult_mana)
var skill_mana: float = 0:
	set(value):
		if enabled:
			skill_mana = value
			max_mana = ActorLibs.calculate_stati(base_mana + skill_mana, max_mana, mult_mana)
var mod_mana: float = 0 :
	set(value):
		if enabled:
			mod_mana = value
			max_mana = ActorLibs.calculate_stati(base_mana + skill_mana, max_mana, mult_mana)
...
func trigger_lust_stats_change(CurrentLust: int, PreviousLust: int) -> void:
	mod_health += SexLibs.get_stat_with_lustf("health", CurrentLust, PreviousLust)
	mod_stamina += SexLibs.get_stat_with_lustf("stamina", CurrentLust, PreviousLust)
	mod_mana += SexLibs.get_stat_with_lustf("mana", CurrentLust, PreviousLust)
